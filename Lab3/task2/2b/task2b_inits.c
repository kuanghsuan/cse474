 ...
 * Student ID: 2027209
 * Date: 2020/11/19
 * This is the initialized file for showing the "Return-to-Sender" function
 */
...
void UART_Init()
{
    ...
    GPIOPCTL |= (0x1 << 24); // Enable port control PA6
    GPIOPCTL |= (0x1 << 28); // Enable port control PA7
    ...
    GPIODIR_A &= ~(0x1 << 6); // set PA6 to input(RXD)
    GPIODIR_A |= (0x1 << 7);  // set PA7 to output(TXD)
}
