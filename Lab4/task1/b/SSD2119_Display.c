...
// ********************************************************
static void LCD_GPIOInit(void){
    // TODO: Complete this function, check the comments
    //       on each line. Create lines of code accordingly.
    ...
    wait++;                                     // wait for port activation
    GPIO_PORTD_DIR_R |= (0x1 << 2);             // make PD2 an output
    GPIO_PORTD_AFSEL_R &= ~(0x1 << 2);          // disable alternate functions
    GPIO_PORTD_DEN_R |= (0x1 << 2);             // enable digital I/O on PD2

    // K5
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R9;    // activate port K
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTK_DIR_R |= (0x1 << 5);             // make PK5 an output
    GPIO_PORTK_AFSEL_R &= ~(0x1 << 5);          // disable alternate functions
    GPIO_PORTK_DEN_R |= (0x1 << 5);             // enable digital I/O on PK5

    // M7
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R11;   // activate port M
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTM_DIR_R |= (0x1 << 7);             // make PM7 an output
    GPIO_PORTM_AFSEL_R &= ~(0x1 << 7);          // disable alternate functions
    GPIO_PORTM_DEN_R |= (0x1 << 7);             // enable digital I/O on PM7

    // N4, N5
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R12;   // activate port N
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTN_DIR_R |= (0x1 << 4);             // make PN4 outputs
    GPIO_PORTN_DIR_R |= (0x1 << 5);             // make PN5 outputs
    GPIO_PORTN_AFSEL_R &= ~(0x1 << 4);          // disable alternate functions
    GPIO_PORTN_AFSEL_R &= ~(0x1 << 5);          // disable alternate functions
    GPIO_PORTN_DEN_R |= (0x1 << 4);             // enable digital I/O on PN4, PN5
    GPIO_PORTN_DEN_R |= (0x1 << 5);             // enable digital I/O on PN4, PN5
    ...
    GPIO_PORTP_DIR_R |= 0x1;                    // make PP0-1 outputs
    GPIO_PORTP_DIR_R |= (0x1 << 1);             // make PP0-1 outputs
    GPIO_PORTP_DIR_R |= (0x1 << 3);             // make PP3-4 outputs
    GPIO_PORTP_DIR_R |= (0x1 << 4);             // make PP3-4 outputs

    GPIO_PORTP_AFSEL_R &= ~(0x1);               // disable alternate functions
    GPIO_PORTP_AFSEL_R &= ~(0x1 << 1);          // disable alternate functions
    GPIO_PORTP_AFSEL_R &= ~(0x1 << 3);          // disable alternate functions
    GPIO_PORTP_AFSEL_R &= ~(0x1 << 4);          // disable alternate functions

    GPIO_PORTP_DEN_R |= 0x1;                    // enable digital I/O on PP0-1, PP3-4
    GPIO_PORTP_DEN_R |= (0x1 << 1);             // enable digital I/O on PP0-1, PP3-4
    GPIO_PORTP_DEN_R |= (0x1 << 3);             // enable digital I/O on PP0-1, PP3-4
    GPIO_PORTP_DEN_R |= (0x1 << 4);             // enable digital I/O on PP0-1, PP3-4
    ...
    GPIO_PORTQ_DIR_R |= 0x1;                    // make PQ0 outputs
    GPIO_PORTQ_DIR_R |= (0x1 << 2);             // make PQ2-3 outputs
    GPIO_PORTQ_DIR_R |= (0x1 << 3);             // make PQ2-3 outputs

    GPIO_PORTQ_AFSEL_R &= ~(0x1);               // disable alternate functions
    GPIO_PORTQ_AFSEL_R &= ~(0x1 << 2);          // disable alternate functions
    GPIO_PORTQ_AFSEL_R &= ~(0x1 << 3);          // disable alternate functions

    GPIO_PORTQ_DEN_R |= 0x1;                    // enable digital I/O on PQ0, PQ2-3
    GPIO_PORTQ_DEN_R |= (0x1 << 2);             // enable digital I/O on PQ0, PQ2-3
    GPIO_PORTQ_DEN_R |= (0x1 << 3);             // enable digital I/O on PQ0, PQ2-3
}
...
void LCD_PrintFloat(float num){
    // TODO: You code here.
    // Note: You don't need to understand technical details
    //       of floating-point numbers. You only need to
